"""Generate GHCPSpend's transparent logo, opaque badge PNG, and scalable SVG."""
import pathlib
import struct
import zlib
...
RADIUS = 9
ASSETS = pathlib.Path(__file__).resolve().parents[1] / "src" / "GHSpend.App" / "Assets"
...
def write_png(path, opaque=False):
    rows = bytearray()
    for y in range(SIZE):
        rows.append(0)
        for x in range(SIZE):
...
if __name__ == "__main__":
    ...
    write_png(ASSETS / "ghcpspend-logo.png")
    write_png(ASSETS / "ghcpspend-badge.png", opaque=True)
    write_svg(ASSETS / "ghcpspend-logo.svg")
