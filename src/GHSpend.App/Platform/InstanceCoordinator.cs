...
internal sealed class InstanceCoordinator : IDisposable
{
    ...
    public static string GetScope(string? portableDirectory)
    {
        using var identity = WindowsIdentity.GetCurrent();
        var user = identity.User?.Value ?? throw new IOException("Could not identify the current Windows user.");
        var scope = user + (portableDirectory is null ? ":installed" : ":portable:" + portableDirectory.ToUpperInvariant());
        return Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(scope)))[..32];
    }

    public static string MutexName(string scope) => @"Global\GHSpend.Runtime." + scope;
    public static string PipeName(string scope) => "GHSpend.Activate." + scope;
    ...
    public static InstanceCoordinator? Start(string scope)
    {
    ...
    public static void Activate(string scope)
    {
        ...
        try
        {
            ...
            if (response[0] != 1)
                throw new IOException("The running GHSpend instance did not accept activation.");
        }
        catch (OperationCanceledException ex)
        {
            throw new IOException("GHSpend is already running but did not respond to activation within five seconds.", ex);
        }
    }
