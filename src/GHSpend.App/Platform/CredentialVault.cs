...
public sealed partial class CredentialVault
{
    ...
    public unsafe string? Read(string target)
    {
        ...
        if (!CredRead(target, Generic, 0, out var credential))
        {
            ...
            if (error == NotFound)
                return null;
            throw new Win32Exception(error, "Could not read the GHSpend credential.");
        }
        ...
        using (credential)
        {
            ...
            if (native->CredentialBlobSize > MaximumBlobBytes || native->CredentialBlobSize % 2 != 0 ||
                ...
                throw new InvalidDataException("The saved GHSpend credential payload is invalid.");
        ...
        try
        {
            ...
            fixed (byte* blobPointer = blob)
            {
                ...
                if (!CredWrite(in credential, 0))
                    throw new Win32Exception(Marshal.GetLastPInvokeError(), "Could not save the GHSpend credential.");
            }
        }
        finally
        {
    ...
    public void Delete(string target)
    {
        ...
        if (!CredDelete(target, Generic, 0))
        {
            var error = Marshal.GetLastPInvokeError();
            if (error != NotFound)
                throw new Win32Exception(error, "Could not delete the GHSpend credential.");
        }
    }
    ...
    private static void ValidateTarget(string target)
    {
        ...
        if (!target.StartsWith("GHSpend/", StringComparison.Ordinal) || target.Length > 32767 || target.Contains('\0'))
            throw new ArgumentException("Credential targets must start with GHSpend/ and contain no null characters.", nameof(target));
    }
