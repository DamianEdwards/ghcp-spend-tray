...
internal sealed unsafe class TrayIcon : IDisposable
{
    ...
    internal TrayIcon(nint owner, string? portableDirectory = null)
    {
        ...
        if (_icon == 0) throw new Win32Exception(Marshal.GetLastPInvokeError(), "Cannot load the GHSpend icon.");
        _data = new Win32.NOTIFYICONDATA
        {
            ...
            guidItem = portableDirectory is null ? new Guid("5d9db52a-41a0-4b3d-910e-031c331ef83a") :
                new Guid(SHA256.HashData(Encoding.UTF8.GetBytes("GHSpend.Tray/" +
    ...
    internal void Add()
    {
        ...
        fixed (char* tip = _data.szTip) Set(tip, 128, "GHSpend | Starting");
