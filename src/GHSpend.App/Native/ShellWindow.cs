...
internal abstract unsafe class ShellWindow : IDisposable
{
    ...
    protected ShellWindow()
    {
        if (!_registered)
        {
            fixed (char* name = "GHSpend.ShellWindow")
            {
        ...
        Handle = Win32.CreateWindowEx(0x80, "GHSpend.ShellWindow", "GHSpend shell host", 0,
    ...
    private static nint WindowProc(nint hwnd, uint message, nuint wParam, nint lParam)
    {
        ...
        catch (Exception ex)
        {
            ...
            Win32.MessageBox(0, "A tray operation failed. Check the diagnostic log.", "GHSpend", Win32.MB_ICONERROR);
        }
