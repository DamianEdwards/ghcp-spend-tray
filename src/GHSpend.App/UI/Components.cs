...
internal static class UI
{
    ...
    internal static Element Logo(double size) => Image(Path.Combine(AppContext.BaseDirectory, "Assets", "ghcpspend-logo.png"))
        .Width(size).Height(size).AutomationName("GHSpend");
    ...
    public override Element Render()
    {
        ...
        return Grid([GridSize.Star()], [GridSize.Auto, GridSize.Star(), GridSize.Auto],
            Grid([GridSize.Auto, GridSize.Star(), GridSize.Auto], [GridSize.Auto],
                ...
                TextBlock("GHSpend").FontSize(18).SemiBold().VAlign(VerticalAlignment.Center).Grid(column: 1),
...
internal sealed class SettingsComponent(AppSession session) : SessionComponent(session)
{
    public override Element Render()
    {
        ...
        return Grid([GridSize.Star()], [GridSize.Auto, GridSize.Star()],
            TitleBar("GHSpend Settings").Grid(row: 0),
    ...
    private Element General(nint owner) => VStack(18,
        UI.Section("Start with Windows", Session.Controller.Portable ?
            "Unavailable in isolated portable mode." : "Launch quietly in the tray when you sign in.",
            ToggleSwitch(Session.Startup, value => { Session.Startup = value; Session.Notify(); })
                .AutomationName("Start with Windows").IsEnabled(!Session.Controller.Portable && !Session.Busy)),
        ...
        UI.Copy("GHSpend runs from its extracted application folder. Keep the complete folder in place; moving it requires updating Start with Windows.")
    );
    ...
    private Element AddForm(nint owner)
    {
        ...
        return VStack(18,
            ...
            UI.Copy("Authorize our GHCPSpend application in your browser. No password or token needs to be pasted here."),
    ...
    private Element AccountDetails(AccountView account, nint owner) => VStack(18,
        ...
        Session.ConfirmRemove
            ? Card(VStack(10, TextBlock("Remove this account from GHSpend?").SemiBold(),
    ...
    private static Element About() => VStack(16, UI.Logo(64).HAlign(HorizontalAlignment.Left),
        TextBlock("GHSpend").FontSize(28).SemiBold(),
