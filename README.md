...
**Pre-release: live authentication and consumption access still require validation.**
GHSpend embeds GitHub CLI's public OAuth client ID; there is no client-ID setting
or bundled client secret. Successful probes with existing tokens do not establish
that new sign-ins with GHSpend's requested scopes will work. See
...
2. Start device sign-in, copy the displayed code, and open the browser.
   **GitHub's consent screen identifies the application as GitHub CLI, not GHSpend.**
...
The fixed public ID is `178c6fc778ccc68e1d6a`, verified against the
[official GitHub CLI authentication source](https://github.com/cli/cli/blob/trunk/internal/authflow/flow.go)
on September 17, 2026. Device authorization, token polling, and refresh use this
same ID on all hosts. No existing `gh` credentials are read and `gh` does not need
to be installed. Legacy configuration client-ID overrides are ignored and omitted
on the next settings save. Accounts authorized under a previous registration may
need **Reconnect**.
...
Using the CLI's ID does not add its normal `repo`, `read:org`, or `gist` scopes.
